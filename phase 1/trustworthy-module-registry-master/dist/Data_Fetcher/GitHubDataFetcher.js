...
class GitHubDataFetcher {
    ...
    async fetchCommits() {
        ...
        const data = await this.fetchData(query, { repoOwner, repoName });
    ...
    async fetchMaintainerMetrics() {
        ...
        const releasesLastYear = repo.recentReleases.nodes.filter((release) => new Date(release.createdAt) > new Date(Date.now() - 365 * 24 * 60 * 60 * 1000)).length;
